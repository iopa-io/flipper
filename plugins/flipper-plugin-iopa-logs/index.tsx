...
export function plugin(client: PluginClient<Events>) {
  ...
  client.onMessage('log', (entry: DeviceLogEntry) => {
...
export function Component() {
  const clientPlugin = usePlugin(plugin);
  return (
    <DataTable<ExtendedLogEntry>
      dataSource={clientPlugin.rows}
      columns={clientPlugin.columns}
      enableAutoScroll
      onRowStyle={getRowStyle}
      enableHorizontalScroll={false}
      extraActions={
        clientPlugin.isConnected ? (
          <>
            <Button title="Clear logs" onClick={clientPlugin.clearLogs}>
              <DeleteOutlined />
            </Button>
          </>
        ) : undefined
      }
      tableManagerRef={clientPlugin.tableManagerRef}
    />
  );
}
